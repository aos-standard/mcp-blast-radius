...
from aos_validator_mcp.precedents_tools import (
    ChainType,
    get_precedent as run_get_precedent,
    list_precedents as run_list_precedents,
    search_by_dna as run_search_by_dna,
)
...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "when divergences exist. Bundled business-DNA precedents: list_precedents, "
        "search_by_dna, get_precedent (declared per-run pricing; not enforced in v0.1)."
    ),
    json_response=True,
)
...
def list_precedents() -> dict[str, Any]:
    """List bundled business-DNA precedents (metadata only, no charge in v0.1)."""
    return run_list_precedents()


@mcp.tool()
def search_by_dna(chain_type: ChainType) -> dict[str, Any]:
    """Filter precedents by DNA chain type (evidence, posting, approval, etc.)."""
    return run_search_by_dna(chain_type)


@mcp.tool()
def get_precedent(precedent_id: str) -> dict[str, Any]:
    """Return full English precedent sections from the bundled wheel (declared per-run)."""
    return run_get_precedent(precedent_id)
