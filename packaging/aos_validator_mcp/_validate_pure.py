...
def _finalize_validation(
    *,
    ...
    blast = build_blast_radius(scan_root, include_peripheral=include_peripheral)
    ...
    *,
    ...
    if target_dir is not None:
        ...
        if agent_path is None:
            ...
            blast = build_blast_radius(root, include_peripheral=include_peripheral)
            return ValidationResult(
                tool_id=tool_id,
                scanned_at=scanned_at,
                aos_score=0.0,
...
def format_text_result(result: ValidationResult) -> str:
    ...
    if result.blast_radius:
        ...
        lines.append(f"blast_radius: python_files={br.get('python_files', 0)} "
                     ...
                     f"excluded_files={br.get('excluded_file_count', 0)}")
        scope = br.get("scan_scope")
        if scope:
            lines.append(f"scan_scope: {scope}")
